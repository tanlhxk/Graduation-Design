...
namespace Game.Camera
{
    public class CameraController : MonoBehaviour
    {
        ...
        public bool invertDrag = true;                 // true: 地图跟随鼠标
        ...
        private Vector3 lastWorldDragPoint;             // 上一帧鼠标在地面上的世界坐标
        ...
        private void HandleInput()
        {
            ...
            {
                isDragging = false;
                lastWorldDragPoint = Vector3.zero;
            }
        }
        ...
        /// <summary>
        /// 获取屏幕坐标点在地面平面（Y=0）上的世界坐标
        /// </summary>
        private Vector3 GetWorldPointOnGroundPlane(Vector2 screenPos)
        {
            Ray ray = mainCamera.ScreenPointToRay(screenPos);
            Plane groundPlane = new Plane(Vector3.up, 0f); // 地面平面 Y=0
            if (groundPlane.Raycast(ray, out float distance))
            {
                return ray.GetPoint(distance);
            }
            return Vector3.zero; // 射线未击中（例如鼠标指向天空）时返回零向量
        }
    }
}